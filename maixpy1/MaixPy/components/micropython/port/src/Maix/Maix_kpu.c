...
STATIC mp_obj_t py_kpu_class_run_yolo2(size_t n_args, const mp_obj_t *pos_args, mp_map_t *kw_args)
{
	...
	if(sipeed_kpu_model_get_input_shape(kpu_net->kmodel_ctx, &w0, &h0, &ch0) != SIPEED_KPU_ERR_NONE)
	{
	    mp_raise_ValueError("[MAIXPY]kpu: first layer not conv layer!\r\n");
            //cyj add 2019-9-23
            mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: first layer not conv layer, return = %d\n",mp_const_none);
	...
	if(check_img_format(arg_img, w0, h0, ch0))
	{
	    mp_raise_ValueError("[MAIXPY]kpu: check img format err!\r\n");
            //cyj add 2019-9-23
            mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: check img format err!, return = %d\n",mp_const_none);
	   return mp_const_none;
	}
	/*****************************region prepare*************************************************/
        ...
        if(region_layer_init(&kpu_detect_rl, kpu_net->kmodel_ctx))
	{
	    mp_raise_ValueError("[MAIXPY]kpu: region_layer_init err!\r\n");
            //cyj add 2019-9-23
            mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: region_layer_init err, return = %d\n",mp_const_none);
	    return mp_const_none;
	}
	/*************************************************************************************/
	//cyj add 2019-9-29
        mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: start sipeed_kpu_model_run\n");
        g_ai_done_flag = 0;
        sipeed_kpu_err_t ret = sipeed_kpu_model_run(kpu_net->kmodel_ctx, arg_img->pix_ai, K210_DMA_CH_KPU, ai_done, NULL);
	if (ret != 0)
        {
            //cyj add 2019-9-29
            mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: sipeed_kpu_model_run error =%d\n",ret);
        ...
        //cyj add 2019-9-29
        mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: wait sipeed_kpu_model_run over\n");
        while (!g_ai_done_flag);
        g_ai_done_flag = 0;
	/****************************start region layer***************************************/
        //cyj add 2019-9-23
        mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: start region layer\n");			
	static obj_info_t mpy_kpu_detect_info;
        ...
        mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: mpy_kpu_detect_info.obj_number=%d \n",obj_num);

        //cyj add 2019-9-29 xian zhi num
        if(obj_num > 3) obj_num =3;
        ...
        else
        {
            region_layer_deinit(&kpu_detect_rl);
            //cyj add 2019-9-23
            mp_printf(&mp_plat_print, "[MAIXPY]:kpu-run_yolo2: err, return = %d\n",mp_const_none);
